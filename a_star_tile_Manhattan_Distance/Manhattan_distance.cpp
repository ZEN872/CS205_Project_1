...
int find_distance(int,int,int,int); // find the mahatten Distance 
///////////////////////////////////////////
...
class Puzzle {
   ...
   void Num_correct_in_grid(){
        ...
        Manhattan_distance = 0;// defines the Manhatten Distance 
        int current = 1; 
        for( int i = 0; i < dim;i++){
            for(int j =0; j < dim; j++){
                if(current >= dim * dim && Grid[i][j] == 0  ) // checks if last vale is ZERO
                    ...
                    {Num_correct++;}         
                else{// if the tile is misplace then the Manhatten Distacne is determined. 
    ...
    int Manhattan_distance_() const {return Manhattan_distance;}// returns the value of the Manhattan distance for this puzzle
    ...
    int Manhattan_distance; // value holds the Manhattan distance for this puzzle  
};
...
struct Heuristic {
    bool operator()(const Puzzle& a, const Puzzle& b) const {
        // Puzzle with the lower Manhattan Distance has higher priority
...
int  main(int argc, char *argv[]){
    vector<vector<int>> grid; // define our baic grid 
    int i = 1; 

    if(argc <spaure_size*spaure_size + 1){ // check if the correct input was put in 
        ...
        print_grid(grid);// Prints input Grid 
        Unform_search(grid); // statrts the search 
        //List out the NODE expaned, Max Queue Size, and Depth of Solution. 
